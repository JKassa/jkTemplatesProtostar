...
<title>{{ shop_name }} - Изменения на счёте партнёра</title>
<style>
                      ...
                      <td style="padding: 7px 0">
                        <font size="2" face="Open-sans, sans-serif" color="#555454">
                          <p style="border-bottom: 1px solid #D6D4D4; margin: 3px 0 7px; text-transform: uppercase; font-weight: 500; font-size: 18px; padding-bottom: 10px" data-html-only="1">
                            {% if transaction %}
							На ваш счёт начислены комиссионные {{ sum }}{{ currency.symbol }} за заказ
							{% else %}
							С вашего счёта списано {{ sum }}{{ currency.symbol }} в связи с их выплатой
							{% endif %}
                          </p>
                          <span style="color:#777">
                            На вашем счёте:
							<strong style="color:#333">
							  {% for cash in user_cash %}{{ cash.sum }}{{ cash.currency }} {% endfor %}
							</strong>
							<br/>
							Доступно для вывода:
							<strong style="color:#333">
							  {% if cash_available %}
							  {% for cash in cash_available %}{{ cash.sum }}{{ cash.currency }} {% endfor %}
							  {% else %}
							  0
							  {% endif %}
							</strong>
